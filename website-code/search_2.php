...
$dept_sql = "SELECT * FROM departments ORDER BY abbr asc";
$dept_result = mysqli_query($db, $dept_sql);

//create a list of instructors for the drop down box
$sql = "SELECT i.id, i.fname, i.lname FROM instructors i LEFT JOIN InstructorDept o ON i.id = o.fk_InstructorID LEFT JOIN departments d ON d.id = o.fk_DeptID WHERE o.fk_DeptID = $dept ORDER BY i.lname asc";
...
//error check the submittal
    ...
    function errChk(){
        ...
        if (preg_match("/[0-9]{1,3}/", $instructorID, $dept)){            
...
<body>
    <div id="wrapper">
        ...
        <div id="content">
            ...
            <form method='POST' action='search_2.php'>
            <div class="table_center">
            <table class="search_box" border="1" cellpadding="4">
                <tr><td>Select Department:</td><td><select name="dept"><option value='1'>ALL - All Departments</option>
                    
                <?php 
                while($dept_row = mysqli_fetch_assoc($dept_result)){
                        $deptList = $dept_row['abbr']." - ".$dept_row['name'];//set the value to a var we can use later                        
                        $dept = $dept_row['id'];
                        echo "<option value='$dept'>$deptList</option>";
                    }                 
                        ?>                                                      
                    </select></td>
                    <td><input type="submit" name="narrow" value="Narrow" /></td></tr> 
            </table>
            </div>
            </form>
        ...
        <form method='POST' action='search_2.php'>
            ...
            <table class="search_box" border="1" cellpadding="4">     
                ...
                while($row = mysqli_fetch_assoc($result)){
                        ...
                        <td><input type="hidden" name="dept" value="1">
